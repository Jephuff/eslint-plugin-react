  ...
  create: Components.detect((context, components, utils) => {
    ...
    */
    ...
    function markPropTypesAsDeclared(node, propTypes) {
      ...
      let component = components.get(componentNode);
      ...
      switch (propTypes && propTypes.type) {
        ...
        case 'ObjectExpression':
          const nonSpreadNodes = [];
          for (let i = 0, l = propTypes.properties.length; i < l; i += 1) {
            const curNode = propTypes.properties[i];
            if (curNode.type === 'ExperimentalSpreadProperty') {
              markPropTypesAsDeclared(node, curNode.argument);
            } else {
              nonSpreadNodes.push(curNode);
            }
          }

          if (nonSpreadNodes.length !== propTypes.properties.length) {
            component = components.get(componentNode);
            declaredPropTypes = component && component.declaredPropTypes || declaredPropTypes;
          }

          iterateProperties(nonSpreadNodes, (key, value) => {
            if (!value) {
              ignorePropsValidation = true;
              return;
            }
